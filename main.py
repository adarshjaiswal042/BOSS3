...
auth_users = [5442868227,5685862369,5145411673,5991186407,6358637973]
sudo_users = auth_users
sudo_groups = [-1001799399613]
...
@bot.on_message(filters.command(["piyush"]))
...
async def account_login(bot: Client, m: Message):
    ...
    if user is not None and user not in sudo_users:
        ...
        editable = await m.reply_text(
            "Hello Bruh **I An Piyush Downloader Bot**. I can download videos from **text** file one by one.**\n\nLanguage** : Python**\nFramework** : Pyrogram\n\nSend **TXT** File {Name : Link}")
    input: Message = await bot.listen(editable.chat.id)
    x = await input.download()
    await input.delete(True)
